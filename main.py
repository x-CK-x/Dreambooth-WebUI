...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        type=str,
        default="logs",
        help="directory for logging everything",
    )
    parser.add_argument(
        "--scale_lr",
        type=str2bool,
...
class ImageLogger(Callback):
    ...
    def _testtube(self, pl_module, images, batch_idx, split):
        for k in images:
            ...
            print(f"============================================tag:\t\t{tag}")
            print(f"============================================grid:\t\t{grid}")
            print(f"============================================pl_module.global_step:\t\t{pl_module.global_step}")
            #pl_module.logger.experiment.add_image(tag, grid, global_step=pl_module.global_step)################################################################################################TestTubeLogger.logger.add_image

            new_img_name = f"images-{tag}_{pl_module.global_step}"
            print("IMAGE BEING SAVED TO --------->\t\t", f"{new_img_name}.png")
            print("REMOVING   /   JUST IN CASE", new_img_name.replace('/', ''))
            ...
            ###
            ### DISABLE THE CODE BLOCK BELOW IF YOU WANT TO SAVE THE IMAGES WITH numpy FORMAT
            ###
            try:
                img = Image.fromarray(grid.cpu().detach().numpy()[0])
                print(f"printing numpy image data --->\t\t{img.format}")
                print("CONVERT TO RGB")
                img = img.convert("RGB")
                print("scale to rgb")
                r, g, b = img.split()
                r = r.point(lambda i: round(i * 255))
                g = g.point(lambda i: round(i * 255))
                b = b.point(lambda i: round(i * 255))
                img = Image.merge('RGB', (r, g, b))

                print("DONE!!")
                failure_flag = False
            except:
                print("CANNOT GET NUMPY ARRAY")
            finally:
                print(f"DID the previous ((( NUMPY-COLLECT-TEST ))) FAIL?????\t\t{failure_flag}")
            ...
            if failure_flag:
                ...
                try:
                    img = grid.numpy()
                    print(f"printing numpy image data --->\t\t{img.dtype}")
                    print("CONVERT TO RGB")
                    ...
                    print(f"DID the previous ---SECOND ATTEMPT--- ((( NUMPY-COLLECT-TEST ))) FAIL?????\t\t{failure_flag}")


            # continue to print the data
            failure_flag = True
            try:
                print(f"printing numpy image data --->\t\t{img}")
                failure_flag = False
            except:
                print("CANNOT PRINT NUMPY ARRAY")
            finally:
                print(f"DID the previous ((( PRINT-TEST ))) FAIL?????\t\t{failure_flag}")

            # first ASSUME "PIL IMAGE
            failure_flag = True
            try:
                img.save(f"{new_img_name}.jpg")
                failure_flag = False
            except:
                print("CANNOT SAVE NUMPY ARRAY")
            finally:
                print(f"DID the previous save FAIL?????\t\t{failure_flag}")




            failure_flag = True
            print("failure flag reset")
            try:
                #plt.imsave(fname=f"{new_img_name}.jpg", arr=img)
                #img = Image.fromarray(img)
                #img.save(f"{new_img_name}.jpg")
                np.save(f"{new_img_name}.npy", img)
                failure_flag = False
            except:
                print("CANNOT SAVE NUMPY ARRAY")
            finally:
                print(f"DID the previous save FAIL to save the 2nd time?????\t\t{failure_flag}")
                print("array SAVED in (NUMPY) format!")

            if failure_flag:
                print("not able to extract image from tensor correctly. BETTER LUCK NEXT TIME!!  :/")
            print("CONTINUING TO NEXT EPOCH!")

            #############pytorch_lightning.loggers.WandbLogger.log_image(key=tag, images=grid, step=pl_module.global_step)  ################################################################################################TestTubeLogger.logger.add_image      need self as param?!?!?!?!?!?
            print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
            print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
            print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
...
class CUDACallback(Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        print(f"trainer.root_gpu---################-->\t{trainer.root_gpu}")
        print(f"trainer.root_gpu.imag---################-->\t{trainer.root_gpu.imag}")
        print(f"trainer.root_gpu.real---################-->\t{trainer.root_gpu.real}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"GPU INFO\t\t", trainer_config["gpus"])
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
